...
class xApp : public ofBaseApp{
	...
	public:
		...
		void draw() {
            ...
            camera.begin();
            ofTranslate(0,50,0);
            ofDrawGrid(250, 10.0f, false, false, false, true);
            camera.end();
            ofPopMatrix();

            shader.begin();
            shader.setUniformTexture("tex",fbo.getTextureReference(),0);
            shader.setUniformTexture("map",fbo.getTextureReference(),1);
            ...
            shader.setUniform1f("timer",ofGetElapsedTimef());
               ...
               /*glBegin(GL_QUADS);
                ...
                glTexCoord2f(dw, 0); glVertex3f(dw, 0, 0);
                glTexCoord2f(dw, dh); glVertex3f(dw, dh, 0);
                glTexCoord2f(0,dh);  glVertex3f(0,dh, 0);
               glEnd();*/
            shader.end();
